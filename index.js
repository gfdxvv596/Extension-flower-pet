...
function handlePressStart(e) {
    ...
    const computedStyle = window.getComputedStyle(petContainer);
    startPetPos = { x: parseFloat(computedStyle.left), y: parseFloat(computedStyle.top) };
    ...
    if (isDragging) {
        if (!animationFrameId) {
            animationFrameId = requestAnimationFrame(() => {
                ...
                const maxLeft = (window.innerWidth / scale) - petContainer.offsetWidth;
                const maxTop = (window.innerHeight / scale) - petContainer.offsetHeight;
...
function handlePressEnd() {
    ...
    if (isDragging) {
        ...
        const finalStyle = window.getComputedStyle(petContainer);
        settings.position.left = parseFloat(finalStyle.left);
        settings.position.top = parseFloat(finalStyle.top);
        saveSettingsDebounced();
    } else {
        toggleActionsMenu();
    }
...
function resetPlant() { if (!confirm(t`Resetting will start your progress over with a new plant. Are you sure?`)) return; const settings = getSettings(); Object.assign(settings, { growthPoints: 0, hasBug: false, lastWatered: 0, lastSunlight: 0, position: { top: null, left: null } }); saveSettingsDebounced(); updatePetUI(); updateSettingsUI(); }
...
function createPetUI() {
    ...
    displayArea.addEventListener('mousedown', handlePressStart); displayArea.addEventListener('touchstart', handlePressStart, { passive: false });
...
(function () {
    ...
    window.addEventListener('resize', () => { if(petContainer && getSettings().enabled){ try { const maxLeft = (window.innerWidth / (window.matchMedia("(max-width: 768px)").matches ? 0.6 : 1)) - petContainer.offsetWidth; const maxTop = (window.innerHeight / (window.matchMedia("(max-width: 768px)").matches ? 0.6 : 1)) - petContainer.offsetHeight; petContainer.style.left = `${Math.min(parseFloat(petContainer.style.left), maxLeft)}px`; petContainer.style.top = `${Math.min(parseFloat(petContainer.style.top), maxTop)}px`; } catch(e) {} } });
